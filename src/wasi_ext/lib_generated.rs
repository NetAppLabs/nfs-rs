...
pub const RIGHTS_SOCK_CONNECT: Rights = 0x10000000;
/// Listen for incoming connection on an address
pub const RIGHTS_SOCK_LISTEN: Rights = 0x20000000;
/// Bind an address to a socket
pub const RIGHTS_SOCK_BIND: Rights = 0x40000000;
/// Accept incoming connection
pub const RIGHTS_SOCK_ACCEPT: Rights = 0x80000000;
/// Receive data on a socket
pub const RIGHTS_SOCK_RECV: Rights = 0x100000000;
/// Send data on a socket
pub const RIGHTS_SOCK_SEND: Rights = 0x200000000;
/// Retrieve locally bound address on a socket
pub const RIGHTS_SOCK_ADDR_LOCAL: Rights = 0x400000000;
/// Retrieve remote address on a socket
pub const RIGHTS_SOCK_ADDR_REMOTE: Rights = 0x800000000;
/// Receive datagram on a socket
pub const RIGHTS_SOCK_RECV_FROM: Rights = 0x1000000000;
/// Send datagram on a socket
pub const RIGHTS_SOCK_SEND_TO: Rights = 0x2000000000;
pub type Fd = u32;
#[repr(C)]
...
/// Address pool
pub const FILETYPE_ADDRESS_POOL: Filetype = 8;
#[repr(C)]
#[derive(Copy, Clone, Debug)]
pub struct Dirent {
...
pub unsafe fn args_get(argv: *mut *mut u8, argv_buf: *mut u8) -> Result<()> {
    let rc = wasi_experimental_sockets::args_get(argv, argv_buf);
...
pub unsafe fn args_sizes_get() -> Result<(Size, Size)> {
    ...
    let rc = wasi_experimental_sockets::args_sizes_get(argc.as_mut_ptr(), argv_buf_size.as_mut_ptr());
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
...
pub unsafe fn environ_get(environ: *mut *mut u8, environ_buf: *mut u8) -> Result<()> {
    let rc = wasi_experimental_sockets::environ_get(environ, environ_buf);
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
        Ok(())
...
pub unsafe fn environ_sizes_get() -> Result<(Size, Size)> {
    ...
    let rc = wasi_experimental_sockets::environ_sizes_get(
        environc.as_mut_ptr(),
...
pub unsafe fn clock_res_get(id: Clockid) -> Result<Timestamp> {
    ...
    let rc = wasi_experimental_sockets::clock_res_get(id, resolution.as_mut_ptr());
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
...
pub unsafe fn clock_time_get(id: Clockid, precision: Timestamp) -> Result<Timestamp> {
    ...
    let rc = wasi_experimental_sockets::clock_time_get(id, precision, time.as_mut_ptr());
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
        Ok(time.assume_init())
...
pub unsafe fn fd_advise(fd: Fd, offset: Filesize, len: Filesize, advice: Advice) -> Result<()> {
    let rc = wasi_experimental_sockets::fd_advise(fd, offset, len, advice);
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
        Ok(())
...
pub unsafe fn fd_allocate(fd: Fd, offset: Filesize, len: Filesize) -> Result<()> {
    let rc = wasi_experimental_sockets::fd_allocate(fd, offset, len);
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
        Ok(())
...
pub unsafe fn fd_close(fd: Fd) -> Result<()> {
    let rc = wasi_experimental_sockets::fd_close(fd);
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
        Ok(())
...
pub unsafe fn fd_datasync(fd: Fd) -> Result<()> {
    let rc = wasi_experimental_sockets::fd_datasync(fd);
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
        Ok(())
...
pub unsafe fn fd_fdstat_get(fd: Fd) -> Result<Fdstat> {
    ...
    let rc = wasi_experimental_sockets::fd_fdstat_get(fd, stat.as_mut_ptr());
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
        Ok(stat.assume_init())
...
pub unsafe fn fd_fdstat_set_flags(fd: Fd, flags: Fdflags) -> Result<()> {
    let rc = wasi_experimental_sockets::fd_fdstat_set_flags(fd, flags);
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
        Ok(())
...
pub unsafe fn fd_fdstat_set_rights(
    ...
    let rc = wasi_experimental_sockets::fd_fdstat_set_rights(fd, fs_rights_base, fs_rights_inheriting);
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
        Ok(())
...
pub unsafe fn fd_filestat_get(fd: Fd) -> Result<Filestat> {
    ...
    let rc = wasi_experimental_sockets::fd_filestat_get(fd, buf.as_mut_ptr());
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
        Ok(buf.assume_init())
...
pub unsafe fn fd_filestat_set_size(fd: Fd, size: Filesize) -> Result<()> {
    let rc = wasi_experimental_sockets::fd_filestat_set_size(fd, size);
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
        Ok(())
...
pub unsafe fn fd_filestat_set_times(
    ...
    let rc = wasi_experimental_sockets::fd_filestat_set_times(fd, atim, mtim, fst_flags);
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
        Ok(())
...
pub unsafe fn fd_pread(fd: Fd, iovs: IovecArray<'_>, offset: Filesize) -> Result<Size> {
    ...
    let rc =
        wasi_experimental_sockets::fd_pread(fd, iovs.as_ptr(), iovs.len(), offset, nread.as_mut_ptr());
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
...
pub unsafe fn fd_prestat_get(fd: Fd) -> Result<Prestat> {
    let mut buf = MaybeUninit::uninit();
    let rc = wasi_experimental_sockets::fd_prestat_get(fd, buf.as_mut_ptr());
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
        Ok(buf.assume_init())
...
pub unsafe fn fd_prestat_dir_name(fd: Fd, path: *mut u8, path_len: Size) -> Result<()> {
    let rc = wasi_experimental_sockets::fd_prestat_dir_name(fd, path, path_len);
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
        Ok(())
...
pub unsafe fn fd_pwrite(fd: Fd, iovs: CiovecArray<'_>, offset: Filesize) -> Result<Size> {
    ...
    let rc = wasi_experimental_sockets::fd_pwrite(
        fd,
        iovs.as_ptr(),
        iovs.len(),
        offset,
...
pub unsafe fn fd_read(fd: Fd, iovs: IovecArray<'_>) -> Result<Size> {
    let mut nread = MaybeUninit::uninit();
    let rc = wasi_experimental_sockets::fd_read(fd, iovs.as_ptr(), iovs.len(), nread.as_mut_ptr());
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
        Ok(nread.assume_init())
...
pub unsafe fn fd_readdir(fd: Fd, buf: *mut u8, buf_len: Size, cookie: Dircookie) -> Result<Size> {
    ...
    let rc = wasi_experimental_sockets::fd_readdir(fd, buf, buf_len, cookie, bufused.as_mut_ptr());
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
...
pub unsafe fn fd_renumber(fd: Fd, to: Fd) -> Result<()> {
    let rc = wasi_experimental_sockets::fd_renumber(fd, to);
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
        Ok(())
...
pub unsafe fn fd_seek(fd: Fd, offset: Filedelta, whence: Whence) -> Result<Filesize> {
    ...
    let rc = wasi_experimental_sockets::fd_seek(fd, offset, whence, newoffset.as_mut_ptr());
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
...
pub unsafe fn fd_sync(fd: Fd) -> Result<()> {
    let rc = wasi_experimental_sockets::fd_sync(fd);
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
        Ok(())
...
pub unsafe fn fd_tell(fd: Fd) -> Result<Filesize> {
    ...
    let rc = wasi_experimental_sockets::fd_tell(fd, offset.as_mut_ptr());
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
...
pub unsafe fn fd_write(fd: Fd, iovs: CiovecArray<'_>) -> Result<Size> {
    let mut nwritten = MaybeUninit::uninit();
    let rc = wasi_experimental_sockets::fd_write(fd, iovs.as_ptr(), iovs.len(), nwritten.as_mut_ptr());
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
        Ok(nwritten.assume_init())
...
pub unsafe fn path_create_directory(fd: Fd, path: &str) -> Result<()> {
    let rc = wasi_experimental_sockets::path_create_directory(fd, path.as_ptr(), path.len());
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
        Ok(())
...
pub unsafe fn path_filestat_get(fd: Fd, flags: Lookupflags, path: &str) -> Result<Filestat> {
    let mut buf = MaybeUninit::uninit();
    let rc = wasi_experimental_sockets::path_filestat_get(
        fd,
        flags,
        path.as_ptr(),
        path.len(),
...
pub unsafe fn path_filestat_set_times(
    ...
    let rc = wasi_experimental_sockets::path_filestat_set_times(
        fd,
        flags,
        path.as_ptr(),
        path.len(),
...
pub unsafe fn path_link(
    ...
    let rc = wasi_experimental_sockets::path_link(
        old_fd,
        old_flags,
        old_path.as_ptr(),
        old_path.len(),
...
pub unsafe fn path_open(
    ...
    let rc = wasi_experimental_sockets::path_open(
        fd,
        dirflags,
        path.as_ptr(),
        path.len(),
...
pub unsafe fn path_readlink(fd: Fd, path: &str, buf: *mut u8, buf_len: Size) -> Result<Size> {
    let mut bufused = MaybeUninit::uninit();
    let rc = wasi_experimental_sockets::path_readlink(
        fd,
        path.as_ptr(),
        path.len(),
        buf,
...
pub unsafe fn path_remove_directory(fd: Fd, path: &str) -> Result<()> {
    let rc = wasi_experimental_sockets::path_remove_directory(fd, path.as_ptr(), path.len());
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
        Ok(())
...
pub unsafe fn path_rename(fd: Fd, old_path: &str, new_fd: Fd, new_path: &str) -> Result<()> {
    let rc = wasi_experimental_sockets::path_rename(
        fd,
        old_path.as_ptr(),
        old_path.len(),
        new_fd,
...
pub unsafe fn path_symlink(old_path: &str, fd: Fd, new_path: &str) -> Result<()> {
    let rc = wasi_experimental_sockets::path_symlink(
        old_path.as_ptr(),
        old_path.len(),
        fd,
        new_path.as_ptr(),
...
pub unsafe fn path_unlink_file(fd: Fd, path: &str) -> Result<()> {
    let rc = wasi_experimental_sockets::path_unlink_file(fd, path.as_ptr(), path.len());
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
        Ok(())
...
pub unsafe fn poll_oneoff(
    ...
    let rc = wasi_experimental_sockets::poll_oneoff(r#in, out, nsubscriptions, nevents.as_mut_ptr());
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
...
pub unsafe fn proc_exit(rval: Exitcode) {
    wasi_experimental_sockets::proc_exit(rval);
}
...
pub unsafe fn proc_raise(sig: Signal) -> Result<()> {
    let rc = wasi_experimental_sockets::proc_raise(sig);
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
        Ok(())
...
pub unsafe fn sched_yield() -> Result<()> {
    let rc = wasi_experimental_sockets::sched_yield();
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
        Ok(())
...
pub unsafe fn random_get(buf: *mut u8, buf_len: Size) -> Result<()> {
    let rc = wasi_experimental_sockets::random_get(buf, buf_len);
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
        Ok(())
...
/// ## Return
///
/// * `bufused` - The number of bytes stored in the buffer. If less than the size of the buffer, no
...
pub unsafe fn addr_resolve(
    ...
    let mut bufused = MaybeUninit::uninit();
    let rc = wasi_experimental_sockets::addr_resolve(
        host.as_ptr(),
        host.len(),
        port,
        buf,
...
pub unsafe fn sock_addr_local(fd: Fd, buf: *mut u8, buf_len: Size) -> Result<()> {
    let rc = wasi_experimental_sockets::sock_addr_local(fd, buf, buf_len);
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
        Ok(())
...
pub unsafe fn sock_addr_remote(fd: Fd, buf: *mut u8, buf_len: Size) -> Result<()> {
    let rc = wasi_experimental_sockets::sock_addr_remote(fd, buf, buf_len);
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
        Ok(())
...
pub unsafe fn sock_open(af: AddressFamily, socktype: SockType) -> Result<Fd> {
    let mut fd = MaybeUninit::uninit();
    let rc = wasi_experimental_sockets::sock_open(af, socktype, fd.as_mut_ptr());
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
        Ok(fd.assume_init())
...
pub unsafe fn sock_close(fd: Fd) -> Result<()> {
    let rc = wasi_experimental_sockets::sock_close(fd);
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
        Ok(())
...
pub unsafe fn sock_set_reuse_addr(fd: Fd, reuse: u8) -> Result<()> {
    let rc = wasi_experimental_sockets::sock_set_reuse_addr(fd, reuse);
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
        Ok(())
...
pub unsafe fn sock_get_reuse_addr(fd: Fd) -> Result<u8> {
    ...
    let rc = wasi_experimental_sockets::sock_get_reuse_addr(fd, reuse.as_mut_ptr());
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
...
pub unsafe fn sock_set_reuse_port(fd: Fd, reuse: u8) -> Result<()> {
    let rc = wasi_experimental_sockets::sock_set_reuse_port(fd, reuse);
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
        Ok(())
...
pub unsafe fn sock_get_reuse_port(fd: Fd) -> Result<u8> {
    let mut reuse = MaybeUninit::uninit();
    let rc = wasi_experimental_sockets::sock_get_reuse_port(fd, reuse.as_mut_ptr());
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
        Ok(reuse.assume_init())
...
pub unsafe fn sock_set_recv_buf_size(fd: Fd, size: Size) -> Result<()> {
    let rc = wasi_experimental_sockets::sock_set_recv_buf_size(fd, size);
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
        Ok(())
...
pub unsafe fn sock_get_recv_buf_size(fd: Fd) -> Result<Size> {
    ...
    let rc = wasi_experimental_sockets::sock_get_recv_buf_size(fd, size.as_mut_ptr());
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
        Ok(size.assume_init())
...
pub unsafe fn sock_set_send_buf_size(fd: Fd, size: Size) -> Result<()> {
    let rc = wasi_experimental_sockets::sock_set_send_buf_size(fd, size);
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
        Ok(())
...
pub unsafe fn sock_get_send_buf_size(fd: Fd) -> Result<Size> {
    let mut size = MaybeUninit::uninit();
    let rc = wasi_experimental_sockets::sock_get_send_buf_size(fd, size.as_mut_ptr());
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
        Ok(size.assume_init())
...
pub unsafe fn sock_bind(fd: Fd, addr: *mut Addr) -> Result<()> {
    let rc = wasi_experimental_sockets::sock_bind(fd, addr);
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
        Ok(())
...
pub unsafe fn sock_listen(fd: Fd, backlog: Size) -> Result<()> {
    let rc = wasi_experimental_sockets::sock_listen(fd, backlog);
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
        Ok(())
...
pub unsafe fn sock_accept(fd: Fd) -> Result<Fd> {
    ...
    let rc = wasi_experimental_sockets::sock_accept(fd, childfd.as_mut_ptr());
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
...
pub unsafe fn sock_connect(fd: Fd, addr: *mut Addr) -> Result<()> {
    let rc = wasi_experimental_sockets::sock_connect(fd, addr);
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
        Ok(())
...
pub unsafe fn sock_recv(fd: Fd, buf: *mut u8, buf_len: Size, flags: Riflags) -> Result<Size> {
    let mut bufused = MaybeUninit::uninit();
    let rc = wasi_experimental_sockets::sock_recv(fd, buf, buf_len, flags, bufused.as_mut_ptr());
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
        Ok(bufused.assume_init())
...
pub unsafe fn sock_recv_from(
    ...
    let mut bufused = MaybeUninit::uninit();
    let rc = wasi_experimental_sockets::sock_recv_from(
        fd,
        buf,
        buf_len,
        addr_buf,
...
pub unsafe fn sock_send(fd: Fd, buf: *mut u8, buf_len: Size, flags: Siflags) -> Result<Size> {
    let mut bufused = MaybeUninit::uninit();
    let rc = wasi_experimental_sockets::sock_send(fd, buf, buf_len, flags, bufused.as_mut_ptr());
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
        Ok(bufused.assume_init())
...
pub unsafe fn sock_send_to(
    ...
    let mut bufused = MaybeUninit::uninit();
    let rc =
        wasi_experimental_sockets::sock_send_to(fd, buf, buf_len, addr, flags, bufused.as_mut_ptr());
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
        Ok(bufused.assume_init())
...
pub unsafe fn sock_shutdown(fd: Fd, how: Sdflags) -> Result<()> {
    let rc = wasi_experimental_sockets::sock_shutdown(fd, how);
    if let Some(err) = Error::from_raw_error(rc) {
        Err(err)
    } else {
        Ok(())
...
pub mod wasi_experimental_sockets {
    use super::*;
    #[link(wasm_import_module = "wasi_experimental_sockets")]
    extern "C" {
        /// Read command-line argument data.
        /// The size of the array should match that returned by `args_sizes_get`
        ...
        ///
        /// Note: This is similar to `socket` in POSIX using PF_INET
        pub fn sock_open(af: AddressFamily, socktype: SockType, fd: *mut Fd) -> Errno;
        /// Close a socket (this is an alias for `fd_close`)
        /// Note: This is similar to `close` in POSIX.
